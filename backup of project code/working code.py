#this code works as single box detection detection without centre representation using YOLOv4 model
...
detected_vehicles = {}     # To track detected vehicles and their IDs
vehicle_positions = {}      # To track the positions of vehicles
...
while True:
            ...
            for obj in detection:
                ...
                if confidence > 0.5:  # Confidence threshold
                    ...
                    if label in ['car', 'truck', 'bus', 'motorcycle']:  # Filter vehicle classes
                        current_detections.append((class_id, (x, y, w, h), confidence))

        # Reset the detected vehicles for the new frame
        detected_vehicles.clear()

        # Update detected vehicles with current detections
        for class_id, (x, y, w, h), confidence in current_detections:
            vehicle_center = (x + w // 2, y + h // 2)
            matched = False

            # Check if the vehicle is already detected and update position
            for vid, (vx, vy, vw, vh) in list(detected_vehicles.items()):
                prev_center = (vx + vw // 2, vy + vh // 2)

                # Check if the previous vehicle is still detected
                if (abs(vehicle_center[0] - prev_center[0]) < 50 and 
                    abs(vehicle_center[1] - prev_center[1]) < 50):  # Adjust distance threshold as necessary
                    matched = True
                    detected_vehicles[vid] = (x, y, w, h)  # Update position
                    break

            if not matched:
                # Assign a new ID for a new vehicle
                new_id = len(detected_vehicles) + 1
                detected_vehicles[new_id] = (x, y, w, h)  # Add new vehicle

        # Draw detected vehicles for the current frame and check for crossings
        for vid, (x, y, w, h) in detected_vehicles.items():
            vehicle_center = (x + w // 2, y + h // 2)

            # Check if the vehicle has crossed the line
            if vehicle_center[1] > min(point1[1], point2[1]) and vehicle_center[1] < max(point1[1], point2[1]):
                if vehicle_center[0] > min(point1[0], point2[0]) and vehicle_center[0] < max(point1[0], point2[0]):
                    # Only count the vehicle if it has not been counted before
                    if vid not in crossed_vehicles:
                        vehicle_count += 1
                        crossed_vehicles.add(vid)

            # Draw the bounding box for the vehicle
            cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
            cv2.putText(frame, f"Vehicle {vid}", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    ...
    if key == ord('q'):  # Quit
        ...
        vehicle_count = 0
        crossed_vehicles.clear()  # Clear the set of crossed vehicles
        detected_vehicles.clear()  # Clear the detected vehicles
